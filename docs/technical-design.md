...
- 账号元数据进入应用 SQLite，例如邮箱地址、IMAP host/port、用户名、连接状态、同步状态、上次同步时间
- 密码、授权码、token 等敏感凭据不进入 JSON 配置文件，也不进入普通应用表字段
- 当前 macOS 桌面实现使用系统钥匙串保存 IMAP 凭据，service 层只在需要同步时读取
