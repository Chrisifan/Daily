...
interface AppNavProps {
  ...
  onPendingSchedulesClick?: () => void;
}

export function AppNav({
  onSettingsClick,
  pendingScheduleCount = 0,
  onPendingSchedulesClick,
}: AppNavProps) {
  const { t } = useTranslation();
  const location = useLocation();
  const pendingLabel = t("integrations.externalCandidate.pendingTooltip");
  const pendingCountLabel = pendingScheduleCount > 9 ? "9+" : String(pendingScheduleCount);
    ...
    <div className="nav-root">
      ...
      <header className="nav-header" data-tauri-drag-region>
        ...
        <nav className="nav-capsule">
          ...
          {pendingScheduleCount > 0 && (
            <button
              onClick={onPendingSchedulesClick}
              className="nav-link nav-link--pending"
              data-tooltip={pendingLabel}
              aria-label={`${pendingLabel} (${pendingScheduleCount})`}
            >
              <IconPendingSchedule />
              <span className="nav-link__badge">{pendingCountLabel}</span>
            </button>
          )}
          <button
